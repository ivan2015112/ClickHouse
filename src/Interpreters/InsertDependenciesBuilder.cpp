...
Chain InsertDependenciesBuilder::createRetry(const std::vector<StorageIDMaybeEmpty> & path, const std::string & partition) const
{
    //PreSink
    //Sink
    //PostSink
    ...
    LOG_DEBUG(logger, "Creating retry chain for path {}", fmt::join(path_str, " -> "));
...
{
    return createRetry(info.getVisitedViews(), info.getLastPartitionChoice());
}
