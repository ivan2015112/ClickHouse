...
void AddDeduplicationInfoTransform::transform(Chunk & chunk)
{
    ...
    info->setOriginalBlock(input.getHeader().cloneWithColumns(chunk.getColumns()), insert_dependencies);

    auto self_filter = info->filterSelfDuplicate();
    if (self_filter.removed_count > 0)
    {
        auto result_chunk = Chunk(self_filter.block.getColumns(), self_filter.block.rows());
        self_filter.tokens->setOriginalBlock(self_filter.block, insert_dependencies);
        result_chunk.getChunkInfos().add(self_filter.tokens);
        result_chunk.getChunkInfos().appendIfUniq(std::move(chunk.getChunkInfos()));
        chunk = std::move(result_chunk);
    }
}
